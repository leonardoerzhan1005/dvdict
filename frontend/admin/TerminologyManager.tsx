...
export const TerminologyManager: React.FC = () => {
  ...
  const handleEdit = useCallback(async (term: AdminTermResult) => {
    if (term.id) {
      try {
        ...
        alert('Ошибка при загрузке термина для редактирования');
      }
    } else {
  ...
  const handleDelete = useCallback(async (id: number | string) => {
    if (confirm('Are you sure you want to purge this record? This action is permanent.')) {
      try {
  ...
  const handleSave = useCallback(async (data: AdminTermResult) => {
    try {
      ...
      alert('Ошибка при сохранении термина: ' + (error as Error).message);
    }
  }, [selectedTerm, refreshTerms]);
  ...
  if (isLoading) {
    return (
      <div className="max-w-[1440px] mx-auto animate-fade-up flex items-center justify-center py-20">
        <div className="text-white text-lg">Загрузка терминов...</div>
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            <div className="flex items-center gap-3 mb-4">
               ...
               <span className="text-[11px] font-black text-orange-500 uppercase tracking-[0.4em]">Node / Archive</span>
            </div>
            <h1 className="text-5xl md:text-6xl font-black text-white serif-modern tracking-tighter">Term Index</h1>
            <p className="text-zinc-400 font-medium mt-3 text-lg">Cross-referencing {terms.length} trilingual mappings in the master terms.</p>
          </div>
          <button 
            onClick={handleAdd}
          ...
          >
            ...
            <span className="relative z-10">Initialize New Entry</span>
          </button>
        </div>
        ...
        <div className="bg-[#121214] border border-white/5 rounded-[3.5rem] overflow-hidden shadow-sm">
          <div className="p-10 border-b border-white/5 bg-zinc-900/30">
            <div className="relative">
              <input 
                type="text"
                placeholder="Filter by keyword, translation, or metadata..."
                value={search}
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-zinc-900/50">
                  <th className="px-12 py-8 text-[11px] font-black text-zinc-400 uppercase tracking-widest border-b border-white/5">Core Descriptor</th>
                  <th className="px-12 py-8 text-[11px] font-black text-zinc-400 uppercase tracking-widest border-b border-white/5">Local (KK)</th>
                  <th className="px-12 py-8 text-[11px] font-black text-zinc-400 uppercase tracking-widest border-b border-white/5">Local (RU)</th>
                  <th className="px-12 py-8 text-[11px] font-black text-zinc-400 uppercase tracking-widest border-b border-white/5">Category</th>
              ...
              <tbody className="divide-y divide-white/5">
                {filteredTerms.length > 0 ? filteredTerms.map((term) => (
                  <tr key={term.id} className="hover:bg-white/5 transition-all group">
                    ...
                    <td className="px-12 py-10 text-zinc-300 font-bold">{term.translations.kk || <span className="text-zinc-600 italic font-medium">Undefined</span>}</td>
                    <td className="px-12 py-10 text-zinc-300 font-bold">{term.translations.ru || <span className="text-zinc-600 italic font-medium">Undefined</span>}</td>
                    <td className="px-12 py-10">
                      <span className="px-4 py-1.5 bg-zinc-900 text-zinc-400 rounded-full text-[10px] font-black uppercase tracking-widest border border-white/5">{term.category || 'Uncategorized'}</span>
                    </td>
                    <td className="px-12 py-10">
                  ...
                  <tr>
                    <td colSpan={5} className="px-12 py-32 text-center">
                      <div className="max-w-xs mx-auto space-y-4">
                        ...
                        <p className="text-zinc-400 font-bold uppercase text-[11px] tracking-widest">Repository is currently empty</p>
                        <button onClick={handleAdd} className="text-orange-500 font-black text-xs uppercase tracking-widest hover:text-orange-400 transition-colors">Add first record</button>
                      </div>
                    </td>
                  </tr>
                )}
