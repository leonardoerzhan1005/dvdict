...
export const AdminAnalytics: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto animate-in fade-in duration-500">
      <div className="mb-8">
        <h1 className="text-3xl font-semibold text-white">Linguistic <span className="text-zinc-500">Analytics</span></h1>
        <p className="text-zinc-400 text-sm mt-2">Deep insights into terminology usage patterns</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-[#121214] border border-white/5 rounded-2xl p-10">
          <div className="flex justify-between items-center mb-10">
            <h3 className="text-xl font-bold text-white">Search Volume Trends</h3>
        ...
        <div className="bg-[#121214] border border-white/5 rounded-2xl p-10 flex flex-col justify-between">
          <div className="space-y-6">
            <h3 className="text-xl font-bold text-white">Language Affinity</h3>
            <div className="space-y-8">
              {[
                { lang: 'English (EN)', perc: 45, color: 'bg-blue-500' },
                { lang: 'Kazakh (KK)', perc: 35, color: 'bg-emerald-500' },
                { lang: 'Russian (RU)', perc: 20, color: 'bg-orange-500' }
              ].map((l) => (
          ...
          <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mt-12">
            Based on 12.4k search events this month
          </p>
        </div>
      </div>
      ...
      <div className="bg-[#121214] border border-white/5 rounded-2xl overflow-hidden mt-8">
        <div className="p-8 border-b border-white/5 flex justify-between items-center">
          <h3 className="text-xl font-bold text-white">Top Searched Terms</h3>
          <button className="text-[10px] font-black text-orange-500 uppercase tracking-widest hover:text-orange-400 transition-colors">Export CSV</button>
        </div>
        <table className="w-full text-left">
          <thead className="bg-zinc-900/50">
            <tr>
              <th className="px-8 py-4 text-[10px] font-black text-zinc-400 uppercase tracking-widest">Term Identity</th>
              <th className="px-8 py-4 text-[10px] font-black text-zinc-400 uppercase tracking-widest">Global Requests</th>
              <th className="px-8 py-4 text-[10px] font-black text-zinc-400 uppercase tracking-widest">Momentum</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {topTerms.map((term, i) => (
              <tr key={i} className="hover:bg-white/5 transition-colors">
                ...
                <td className="px-8 py-6 text-zinc-400 font-medium">{term.searches.toLocaleString()} hits</td>
